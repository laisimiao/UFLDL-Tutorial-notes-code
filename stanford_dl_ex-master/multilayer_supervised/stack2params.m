...
for d = 1:numel(stack)
    ...
    % Check that stack is of the correct form --> assert(cond,msg):如果 cond 为 false，assert(cond,msg) 会引发错误并显示错误消息 msg。
    assert(size(stack{d}.W, 1) == size(stack{d}.b, 1), ...
     ...
     if d < numel(stack)
        assert(mod(size(stack{d+1}.W, 2), size(stack{d}.W, 1)) == 0, ...  %除后的余数（取模运算）
